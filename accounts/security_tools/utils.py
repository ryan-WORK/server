...
def _make_cache_key(group, rate, value, methods):
    ...
    prefix = getattr(settings, 'SECURITYTOOLS_CACHE_PREFIX', 'rl:')
    return prefix + hashlib.md5(u''.join(parts).encode('utf-8')).hexdigest()


def is_account_blocked(request, group=None, fn=None, key=None, rate=None,
    ...
    if not getattr(settings, 'SECURITYTOOLS_ENABLE', True):
    ...
    fail_open = getattr(settings, 'SECURITYTOOLS_FAIL_OPEN', False)
...
def get_usage_count(request, group=None, fn=None, key=None, rate=None,
                    method=ALL, increment=False):
    if not key:
        raise ImproperlyConfigured('Issuse is that key must be specified')
    limit, period = _split_rate(rate)
    cache_name = getattr(settings, 'SECURITYTOOLS_USE_CACHE', 'default')
    ...
    if callable(key):
        ...
        if accessor not in _ACCESSOR_KEYS:
            raise ImproperlyConfigured('Unknown security_tools key: %s' % key)
        ...
        raise ImproperlyConfigured(
            'Could not understand security_tools key: %s' % key)
...
is_account_blocked.ALL = ALL
is_account_blocked.UNSAFE = UNSAFE
